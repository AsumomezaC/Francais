#Idioma #Francés #LenguaRomance #Comunicación #Humano 

> [!tip] Verbo fairer
> En francés muchas veces se sobre usa el verbo visitar, pe. en vez de decir 'visitar' dicen 'hacer una visita'


> [!warning] Los verbos nunca realizan liasen sin importar su conjugación.

# Útiles
il y a -> hay
	il n'y a -> no hay
# Grupos
Se dividen en 4 grupos principales:
- [[#Verbos -er]]
- [[#Verbos -ir]]
- [[#Verbos del tercer grupo]]
- [[#Irregulares]]
## Categorías extra
- [[#Verbo pronominales]]
# Categoría
Se refiere a como termina un verbo. 
>No todos los verbos de la misma categoría son del mismo [[#Grupos|grupo]]
# Verbos -er
Se quita la parte de 'er', y se adjunta:
- je/j' -> +e
- tu/t' -> +es
- il/elle/on ->  +e
- ils/elles -> +ent
- nous -> +ons
- vous -> +ez
>El 'on' es el nosotros informal hablado, así como el pronombre en oraciones equivalentes al 'se' en 'se habla francés'

adore -> adorar
aimer -> amar/gustar/querer
apprendre -> aprender
arriver -> llegar
bosser -> trabajar / chambear
chercher -> buscar
continuer -> continuar
écouter -> escuchar
étudier -> estudiar
detester -> detestar
dîner -> cenar
Goûter, comer la colación 
habiter -> habitar
jouer -> jugar (como el 'play' del [[Inglés]])
longer -> rodear
manger -> comer
marcher -> caminar
	marcher jusq'à -> caminar hasta
monter -> subir
parler -> hablar
participer -> participar
pratiquer -> practicar
preparer -> preparar
	se puede hacer [[#Verbo pronominales|pronominal]]
profiter -> disfrutar
tourner -> girar (como 'turn' en [[Inglés]])
travailler -> trabajar
trouver -> encontrar/buscar
	[[Preguntas y Respuestas - Francés]] -> oú se trouve...? -> dónde se encuentra?
regarder -> ver algo en una pantalla/observar (igual que el 'watch' en [[Inglés]])
réviser -> estudiar
voyager -> viajar
### Verbes en -cer
Se conjugan igual que [[#Verbos -er]], exceptuando el nous: se conjuga '-çons'
- Commencer
- déplacer
- placer
- annoncer
# Verbo pronominales
>Son aquellos que se acompañan de un pronombre
>>Como llamar, nosotros no decimos 'Yo llamó X', se dice 'Yo ==me== llamó'
>>Se usa para un verbo que se puede aplicar para otra persona o para el sujeto de la acción

**Auxiliares**:
- me, je
- te, tu
- se, elle, il, on
- nous,nous
- vous, vous
- se, elles, ils

Se conjugan igual que los [[#Verbos -er]]
- appeller -> llamar
- promener -> pasear
	si se usa como sustantivo se usa 'faire une pomenade'
- visiter -> visitar
- Laver -> lavar/bañarse
- Doucher -> ducharse
- Coucher -> acostar
- Lever, levantar
- Réveiller, despertar
- Brosser, cepillar
- habiller, vestirse
- préparer, prepararse
- reposer, descansar
- détendre, detenerse/hacer una pausa
- changer, cambiar
- endormir, dormir


	
> [!NOTE] Pronombre
> me -> 'je'
> se -> 'il/elle/on'
> te -> 'tu'
> se -> 'ils/elles'
> nous -> 'nous'
> vous -> 'vous'
> vous -> 'vous (usted)'
> >Si algún verbo empieza con vocal se realiza una contracción (s', m', t') o se hace la liaison.
> >Ojo: se repite la forma del ==pronombre==
# Verbos -ir
- sortir -> salir
# Verbos del tercer grupo
Son aquellos que no pertenecen ni al primer ni al segundo [[#Grupos]], pero hay más de dos verbos que se conjugan igual (a diferencia de los [[#Irregulares]]).
### Descendre
>Descender / Bajar
>
- je -> descends 
- tu/t' -> descends
- il/elle/on ->  descend
- nous -> descendons
- vous -> descendez
- - ils/elles -> descendent
### Prendre
>Tomar
- je -> prends 
- tu/t' -> prends
- il/elle/on ->  prend
- nous -> prenons
- vous -> prenez
- - ils/elles -> prennent
# Irregulares
>Irréguliers
## être
>Ser o estar
>	para nacionalidad, localiser, adjectifs, présenter (con c'est)
- je -> suis
- tu -> es
>No se hace nunca la liaison de 's'
- il/elle/on/c' ->  est
- ils/elles -> sont
- nous -> sommes
>No se pronuncia la 'mes'
- vous -> êtes
## Avoir
>Tener
>	para edad, para posesión (incluye familia)
- j' -> ai
- tu/t' -> as
- il/elle/on ->  a
- ils/elles -> ont
- nous -> avons
- vous -> avez

> [!important] Il y a
> Muy utilizado, y significa 'hay'
> >Negación: Il n'y a pas

## Aller
>ir
- je vais
- tu vas
- il/elle/on va
- nous allons
- vous allez
- ils/elles vont
## Venir
>Venir
- je viens
- tu viens
- il/elle/on vient
- nous venons
- vous venez
- ils/elles viennent
>Ver reglas de acompañamiento [[Locación- Francés#Caso 2|aquí para países o continentes]] y [[Locación- Francés#Origine (origen)|aquí para ciudades]].
## Faire
>Hacer
- je fais
- tu fais
- il/elle/on fait
- nous faisons
- vous faites
- ils/elles font
## Voulior
>Quiero/Pedir algo de forma educada
- je veux
- tu veux
- il/elle/on veut
- nous voulons
- vous voulez
- ils/elles veulent